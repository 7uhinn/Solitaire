...
struct List{
	Cards *top_head;//Top list head
	ListNodes *foundation_head;//points first foundation list
	ListNodes *board_head;//points first board list
	FILE *fptr;
	...
	void printkart(Cards *kart);// prints datas of one kart node
	void printlists();// Prints all lists
	void printmenu();//Prints choice menu
	bool addcardtolist(ListNodes *selectedlist, Cards *transferedcard, int whichlisttype);//Adds transferedcard to selectedlist, whichlisttype is entered by program
	void show_TopToFoundation();//Gets input and calls related function
	void TopToFoundation(char s_color, char s_suit, int s_value);// Moves a card top list to foundation list
	void show_TopToBoard();//Gets input and calls related function
	void TopToBoard(int listindex, char s_color, char s_suit, int s_value);//Moves a card from top list to board list
	void show_BoardToBoard();//Gets input and calls related function
	...
	void show_BoardToFoundation();//Gets input and calls related function
	void BoardToFoundation(int fromwhere);//Moves a last card in fromwhere.list of boardlist  to foundation list
	void opn_opt();//Wants user to select choice from menu, and calls related functions
...
int main()
{
	char inp;
	opn.create();
	if(!opn.fptr)//If cannot find solitaire.txt file, close the program
		return 0;

	opn.istopempty = false;//If the top list is empty, this value will be true
	opn.isboardempty = false;//If the board list is empty, this value will be true
	...
	while(1){
		...
		if(inp == '1'){
			...
			opn.show_BoardToBoard();
			cin.ignore(10000, '\n');
			getchar();
		}
		else if(inp == '4'){
			opn.show_BoardToFoundation();
			cin.ignore(10000, '\n');
		}
		else if(inp == '9'){
			opn.close();
		...
		if(!opn.top_head)// checking top list empty or not
			opn.istopempty = true;
		
		ListNodes *traverse;
		traverse = opn.board_head;
		int counter = 0;
		while(traverse){//checking board lists is empty or not
			...
			if(counter == 7)// counter is 7 if all of 7 boardlists are empty
				opn.isboardempty = true;
		}

		/*When game is completed prints message to user*/
		if(opn.isboardempty && opn.istopempty){//Stop while loop when board and top lists are empty
...
void List::create_boardlists(){
	Cards *newnode, *final;// final points to last node of card list
	ListNodes *newboardlist, *boardlist_final;// boardlist_final points to last node of board list
	char tempcolor, tempsuit, tempvalue[4], tempisup[8], garbage[10];

	int index = 1;// This index represents nth board list
	
	newboardlist = new ListNodes;// creating first boardlist node
...
void List::printkart(Cards *kart){//prints datas of kart node
...
void List::printlists(){
	clear_screen();
	Cards *ct[7];// Board List Card Node Traverser; ct[0] for 1.list, ct[1] for 2.list ....
	Cards *foundct[4];//Found List Card Node Traverser, foundct[0] = Spades, foundct[1] = Hearts, foundct[2] = Diamonds, foundct[3] = Clubs 
	Cards *topct;// TopList Card Traverser
	ListNodes *listtraverse;// List Node Traverser
...
void List::printmenu(){
	...
	cout << "\t4. Move from Board List to Foundation Lists" << endl;
	cout << "Please enter your choice (1, 2, 3, or 4), (enter 9 to exit):" ;
}

bool List::addcardtolist(ListNodes *selectedlist, Cards *transferedcard, int whichlisttype){//whichlisttype is not related with user, it is automatically entered by progra
	Cards *cardtraverser;
...
void List::show_TopToFoundation(){// wants input from use
...
void List::TopToFoundation(char s_color, char s_suit, int s_value){
	ListNodes *listtraverse;
	Cards *willbemoved = NULL, *cardtraverse, *cardtail, *temptop_head = top_head;
...
void List::show_TopToBoard(){// wants input from user for moving card top list to board lis
	char Symbol_of_colors, Symbol_of_suits, tempvalue[4];
...
void List::BoardToBoard(int fromwhere, int towhere, char s_color, char s_suit, int s_value){
	ListNodes *sourcelisttraverse = board_head, *targetlisttraverse = board_head;
	Cards *willbemoved = NULL, *cardtraverse, *cardtail, *temp_head;
...
void List::show_BoardToFoundation(){
	int source;
	cout <<  "Select the number of the source Board List:";
	scanf("%d", &source);
	cin.ignore(1000, '\n');
...
void List::BoardToFoundation(int fromwhere){
	ListNodes *sourcelisttraverse = board_head, *targetlisttraverse = foundation_head;
	Cards *willbemoved = NULL, *cardtraverse, *cardtail, *temp_head;
